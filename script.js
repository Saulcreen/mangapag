/* ═══════════════════════════════════════════════════
   ...
   // Proxy para imágenes de portada
   function proxyCover(mangaId, filename) {
     const url = `${MDEX_COVERS}/${mangaId}/${filename}.512.jpg`;
     return `https://corsproxy.io/?${encodeURIComponent(url)}`;
   }
   
   // Proxy para páginas del lector
   ...
   async function loadChapter(idx) {
     ...
     try {
       const data = await fetchMDex(`/at-home/server/${chap.id}`);
       const baseUrl  = data.baseUrl;
       const hash     = data.chapter?.hash;
       const pages    = data.chapter?.data || [];
   
       readerPages = pages.map(p => proxyPage(`${baseUrl}/data/${hash}/${p}`));
